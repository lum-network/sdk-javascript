...
export const Pool = {
    ...
    toJSON(message: Pool): unknown {
        ...
        if (message.icaDepositPortId !== '') {
            obj.icaDepositPortId = message.icaDepositPortId;
        }
        if (message.icaPrizepoolPortId !== '') {
            obj.icaPrizepoolPortId = message.icaPrizepoolPortId;
        }
        if (message.validators?.length) {
            obj.validators = message.validators.map((e) => PoolValidator.toJSON(e));
        }
        if (message.bech32PrefixAccAddr !== '') {
        ...
        if (message.icaDepositAddress !== '') {
            obj.icaDepositAddress = message.icaDepositAddress;
        }
        if (message.icaPrizepoolAddress !== '') {
            obj.icaPrizepoolAddress = message.icaPrizepoolAddress;
        }
        if (!message.nextDrawId.isZero()) {
            obj.nextDrawId = (message.nextDrawId || Long.UZERO).toString();
        }
        if (message.tvlAmount !== '') {
            obj.tvlAmount = message.tvlAmount;
        }
        if (!message.depositorsCount.isZero()) {
            obj.depositorsCount = (message.depositorsCount || Long.UZERO).toString();
        }
        if (message.sponsorshipAmount !== '') {
            obj.sponsorshipAmount = message.sponsorshipAmount;
        }
        if (message.lastDrawCreatedAt !== undefined) {
            obj.lastDrawCreatedAt = message.lastDrawCreatedAt.toISOString();
        }
        if (message.lastDrawState !== 0) {
            obj.lastDrawState = drawStateToJSON(message.lastDrawState);
        }
        if (message.availablePrizePool !== undefined) {
            obj.availablePrizePool = Coin.toJSON(message.availablePrizePool);
        }
        if (message.state !== 0) {
            obj.state = poolStateToJSON(message.state);
        }
        if (!message.createdAtHeight.isZero()) {
